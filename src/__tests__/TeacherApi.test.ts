...
describe("TeacherApi", () => {
  ...
  it("should get teachers", async () => {
    ...
    expect(axiosInstance.get).toHaveBeenCalledWith("/teacher/get-teachers");

    expect(result).toEqual([{ teacher_name: "John" }]);
  });
  ...
  it("should add teacher", async () => {
    ...
    expect(axiosInstance.post).toHaveBeenCalledWith("/teacher/post-teachers", {
      teacher_name: "John",
    });
  ...
  it("should update teacher", async () => {
    ...
    const result = await updateTeacher(1, "John Updated");

    expect(axiosInstance.put).toHaveBeenCalledWith("/teacher/put-teachers/1", {
